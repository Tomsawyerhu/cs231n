...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    _next_h=x@Wx+prev_h@Wh+b
    next_h=np.tanh(_next_h)
    cache=(x,prev_h,Wx,Wh,b,_next_h)
...
def rnn_step_backward(dnext_h, cache):
    ...
    x,prev_h,Wx,Wh,b,_next_h=cache
    dtanh=(1-np.tanh(_next_h)**2)*dnext_h
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(x.shape[1]):
        _h, _cache=rnn_step_forward(x[:,i,:], _h, Wx, Wh, b)
        cache.append(_cache)
        h[:,i,:]=_h
...
def rnn_backward(dh, cache):
    ...
    for i,step_cache in enumerate(reversed(cache)):
        idx=len(cache)-i-1
        _dx, _dprev_h, _dWx, _dWh, _db=rnn_step_backward(dh[:,idx,:]+_dprev_h,step_cache)
        dx=_dx if dx is None else np.concatenate((_dx,dx),axis=1)
        dWx=_dWx if dWx is None else _dWx+dWx
        dWh=_dWh if dWh is None else _dWh+dWh
        db=_db if db is None else _db+db
        
    dx=dx.reshape(dh.shape[0],dh.shape[1],-1)
    dh0=_dprev_h
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    R_4h=x@Wx+prev_h@Wh+b
    a_i,a_f,a_o,a_g=R_4h[:,:H],R_4h[:,H:2*H],R_4h[:,2*H:3*H],R_4h[:,3*H:]
    ...
    cache=(x,Wx,Wh,b,R_4h,i,f,o,g,prev_c,prev_h,next_c,next_h)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x,Wx,Wh,b,R_4h,i,f,o,g,prev_c,prev_h,next_c,next_h=cache
    N,H=prev_h.shape
    a_i,a_f,a_o,a_g=R_4h[:,:H],R_4h[:,H:2*H],R_4h[:,2*H:3*H],R_4h[:,3*H:]
    ...
    dR_4h = np.concatenate((da_i, da_f, da_o, da_g), axis=1)
    dx = dR_4h @ Wx.T
    dprev_h = dR_4h @ Wh.T
    dWx = x.T @ dR_4h
    dWh = prev_h.T @ dR_4h
    db = np.sum(dR_4h, axis=0)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(x.shape[1]):
        _h,_c,_cache=lstm_step_forward(x[:,i,:], _h, _c, Wx, Wh, b)

        if i==0:
            h=_h
            cache=[_cache]
        else:
            h=np.concatenate((h,_h),axis=1)
            cache.append(_cache)
...
def lstm_backward(dh, cache):
    ...
    N, T, H = dh.shape
    D = cache[0][0].shape[1]  # retrieve D from x in the first cache
    ...
    for i in reversed(range(T)):
        _dh = dh[:, i, :] + dh0
        _cache = cache[i]
        _dx, dh0, dnext_c, _dWx, _dWh, _db = lstm_step_backward(_dh, dnext_c, _cache)
        dx[:, i, :] = _dx
        dWx += _dWx
        dWh += _dWh
        db += _db
